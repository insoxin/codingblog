...
function fanly_submit($post_ID, $post) {
	if( isset($_POST['Fanly_Submit_CHECK']) ){
		...
		//判断是否设置新增
		if(get_post_meta($post_ID,'Fanly_Submit',true)!='OK' && get_post_meta($post_ID,'Fanly_Submit',true)!='-1'){
			if($P_Fanly_Submit=='realtime'){update_post_meta($post_ID, 'Fanly_Submit', 'realtime');}else{update_post_meta($post_ID, 'Fanly_Submit', 0);}
		}
		//判断是否设置原创
		if(get_post_meta($post_ID,'original',true)!='OK'){
			if($P_original=='1'){update_post_meta($post_ID, 'original', 1);}else{update_post_meta($post_ID, 'original', 0);}
		}
		//判断文章状态与推送状态 返回/继续
		if( $post->post_status != 'publish' || (get_post_meta($post_ID,'Fanly_Submit',true) == 'OK' && get_post_meta($post_ID,'original',true) == 'OK') )return;
		//执行
		if( $P_Fanly_Submit || $P_original ){
			$type = $P_Fanly_Submit=='batch' ? 'batch' : 'realtime';
			$o_type = $P_original==1 ? ',original' : '';
			$cambrian_api_url = 'http://data.zz.baidu.com/urls?appid='.$Fanly['APPID'].'&token='.$Fanly['APPToken'].'&type='.$type.$o_type;
			...
			if ( is_wp_error( $cambrian_re ) ) {
				...
				if($res['success_original']==1){//原创
					$Fanly['realtime'] = $res['remain_realtime'].'|'.date('Ymd');
					update_post_meta($post_ID, 'original', 'OK');// 原创成功提交
					update_post_meta($post_ID, 'Fanly_Submit', 'OK');// OK 表示成功提交
				}elseif($res['success_'.$type]==1){
...
function fanly_submit_to_publish_metabox() {
	...
	$checked		= $fanly_submit=='0' ? '' : ($Fanly['Default']=='true'||$fanly_submit=='realtime' ? 'checked="checked"' : '');
	$checked_o		= ((get_post_meta($post_ID,'original',true)=='' && $Fanly['Original']=='true') || get_post_meta($post_ID,'original',true)==1 || get_post_meta($post_ID,'original',true)=='OK') ? 'checked="checked"' : '';
	
	if($original=='OK'){
		$original_box = '（<label><input name="original" type="checkbox" value="1" disabled '.$checked_o.'>原创</label>）';
	}else{
		$o = @$remain_z[1]==date('Ymd') ? '['.$remain_z[0].']' : '';
	...
	}elseif($fanly_submit == 'realtime' || $fanly_submit=='0'){//已经勾选的新增数据提交
		$input = '
			<input id="Fanly_Submit" name="Fanly_Submit" type="checkbox" value="realtime" '.$checked.'>
...
function fanly_submit_render_post_columns($column_name, $id) {
    switch ($column_name) {
		case 'Fanly_Submit':
			echo get_post_meta( $id, 'original', TRUE)==1 || get_post_meta( $id, 'original', TRUE)=='OK' ? '是'	: '否'; //原创
			echo '/';
