...
/*分类列表1*/
$cat_tax_name1 = $insoxin['cat3_taxonomy_name1'];
if($cat_tax_name1 == 'post'){
    $post_type1 = 'post';
    $cat_tax1  = 'category';
    $tag_tax1  = 'post_tag';
    $category1  = $insoxin[ 'post_cat3_list1'];
    $tag1       = $insoxin[ 'post_cat3_tag1'];
}else if($cat_tax_name1 == 'gallery'){
    $post_type1 = 'gallery';
    $cat_tax1  = 'gallery-cat';
    $tag_tax1  = 'gallery-tag';
    $category1  = $insoxin[ 'gallery_cat3_list1'];
    $tag1       = $insoxin[ 'gallery_cat3_tag1'];
}else if($cat_tax_name1 == 'video'){
    $post_type1 = 'video';
    $cat_tax1  = 'video-cat';
    $tag_tax1  = 'video-tag';
    $category1  = $insoxin[ 'video_cat3_list1'];
    $tag1       = $insoxin[ 'video_cat3_tag1'];
}

/*分类列表2*/
$cat_tax_name2 = $insoxin['cat3_taxonomy_name2'];
if($cat_tax_name2 == 'post'){
    $post_type2 = 'post';
    $cat_tax2  = 'category';
    $tag_tax2  = 'post_tag';
    $category2  = $insoxin[ 'post_cat3_list2'];
    $tag2       = $insoxin[ 'post_cat3_tag2'];
}else if($cat_tax_name2 == 'gallery'){
    $post_type2 = 'gallery';
    $cat_tax2  = 'gallery-cat';
    $tag_tax2  = 'gallery-tag';
    $category2  = $insoxin[ 'gallery_cat3_list2'];
    $tag2       = $insoxin[ 'gallery_cat3_tag2'];
}else if($cat_tax_name2 == 'video'){
    $post_type2 = 'video';
    $cat_tax2  = 'video-cat';
    $tag_tax2  = 'video-tag';
    $category2  = $insoxin[ 'video_cat3_list2'];
    $tag2       = $insoxin[ 'video_cat3_tag2'];
}
...
<section class="cat wrapper scroll3">
    <section class="cat-wrap<?php if(!wp_is_mobile()) { ?> left<?php } ?>">
        ...
        <section class="box<?php triangle();wow(); ?>">
            ...
            <section class="cat3_list1">
                <ul class="layout_ul">
                    <?php $cat1count1=$insoxin[ 'cat3_count1']; $args=array( 'post_type'=> $post_type1,'posts_per_page' => $cat1count1,'ignore_sticky_posts' => 1,'tax_query' => array( array( 'taxonomy' => $cat_tax1, 'field' => 'id', 'terms' => $cat1list1 ))); $wp_query = new WP_Query( $args );if ( $wp_query->have_posts() ) : $count = 1; while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
                    <li class="layout_li">
        ...
        <section class="box<?php triangle();wow(); ?>">
            ...
            <section class="cat3_list2">
                <ul class="layout_ul">
                    <?php $cat1count2=$insoxin[ 'cat3_count2']; $args=array( 'post_type'=> $post_type2,'posts_per_page' => $cat1count2,'ignore_sticky_posts' => 1,'tax_query' => array( array( 'taxonomy' => $cat_tax2, 'field' => 'id', 'terms' => $cat1list2 ))); $wp_query = new WP_Query( $args );if ( $wp_query->have_posts() ) : $count = 1; while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
                    <li class="layout_li">
    ...
    <aside class="sidebar right">
        <section class="cat3_sidebar">
            <?php dynamic_sidebar( __( '首页分类模块3', 'insoxin') ); ?>
        </section>
    ...
    <div class="sf3"></div>
    <?php } else { ?>
    <aside class="sidebar right"><article class="sidebar_widget widget_insoxin_init box<?php triangle();wow(); ?>"><div class="sidebar_title"><h3><?php _e('温馨提示','insoxin'); ?></h3></div><div class="init"><a href="<?php echo get_home_url(); ?>/wp-admin/widgets.php"><?php _e('请到后台外观——小工具中添加小工具到<br><strong>首页分类模块3</strong>边栏中。','insoxin'); ?></a></div></article></aside>
    <?php } } ?>
</section>
<?php } ?>