...
/* ------------------------------------ */
	function __construct() {
		parent::__construct( false, __('insoxin-每日一句','insoxin'), array('description' => __('每日一句小工具','insoxin'), 'classname' => 'widget_insoxin_word') );;	
	}
...
/* ------------------------------------ */
	public function form($instance) {
		// 默认设置
		$defaults = array(
			'title'          => __('每日一句','insoxin'),
			'word_text'    => __('萨龙网络始建于2012年8月，是一家新生的优秀互联网综合服务提供商！','insoxin'),
			'word_author'    => __('萨龙龙','insoxin'),
		);
...
?>
	...
	.widget .widget-inside .postform { width: 100%; }
		...
		<p>
			<label for="<?php echo $this->get_field_id('word_text'); ?>"><?php _e('内容：','insoxin'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('word_author'); ?>"><?php _e('作者：','insoxin'); ?></label>
...
/*  注册小工具
/* ------------------------------------ */
if ( ! function_exists( 'insoxin_register_widget_Word' ) ) {

	function insoxin_register_widget_Word() { 
		register_widget( 'SlongWord' );
	}
	
}
add_action( 'widgets_init', 'insoxin_register_widget_Word' );
