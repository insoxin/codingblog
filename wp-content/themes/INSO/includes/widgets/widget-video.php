...
/* ------------------------------------ */
	function __construct() {
		parent::__construct( false, __('insoxin-视频聚合','insoxin'), array('description' => __('视频聚合小工具','insoxin'), 'classname' => 'widget_insoxin_video') );;	
	}
    ...
    <ul class="<?php if(!$instance[ 'posts_thumb'] && !$instance['posts_category'] && !$instance['posts_date'] && !$instance['posts_like']) { ?>no-info<?php }else{ ?><?php if(!$instance[ 'posts_thumb']) { ?>no-thumb<?php } ?><?php if(!$instance['posts_category'] && !$instance['posts_date'] && !$instance['posts_like']) { ?> no-info<?php } } ?>">
        ...
        <li>
            ...
            <article class="postlist">
                ...
                <figure>
                    <a href="<?php the_permalink() ?>" title="<?php the_title(); ?><?php if($instance['posts_orderby'] == 'comment_count') { echo '&nbsp;'; printf( _n( '有(1)条评论', '有(%s)条评论', $commentcount, 'insoxin' ), $commentcount ); }else { echo '&nbsp;'; printf( __( '有(%s)人点赞', 'insoxin' ), $votes_count ); } ?>"<?php echo new_open_link(); ?>>
                        <i class="icon-play"></i><?php if( $insoxin['thumb_mode']== 'timthumb'){ post_thumbnail(); }else{ widget_post_thumbnail(); } ?>
                    </a>
                </figure>
                <?php } ?>
                <h3>
...
?>
	...
	.widget .widget-inside .postform { width: 100%; }
		...
		<p>
			...
			<?php wp_dropdown_categories( array( 'name' => $this->get_field_name("posts_cat_id"), 'taxonomy' => 'video-cat', 'selected' => $instance["posts_cat_id"], 'show_option_all' => __('全部','insoxin'), 'show_count' => true ) ); ?>
		</p>
		<p>
...
/*  注册小工具
/* ------------------------------------ */
if ( ! function_exists( 'insoxin_register_widget_Video' ) ) {

	function insoxin_register_widget_Video() { 
		register_widget( 'SlongVideo' );
	}
	
}
add_action( 'widgets_init', 'insoxin_register_widget_Video' );
