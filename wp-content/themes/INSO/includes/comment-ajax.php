...
//评论模板
if ( ! function_exists( 'insoxin_comment' ) ) :
function insoxin_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( '引用:', 'insoxin' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '编辑', 'insoxin' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
	?>
	<li <?php comment_class(); ?>>
		<article id="comment-<?php comment_ID(); ?>" class="comment_body">
            <div class="comment_author">
                <?php $user_id = $comment->user_id; $user_name = $comment->comment_author; echo insoxin_get_avatar($user_id,$user_name); ?>
            </div>
            <div class="comment_info">
                <span class="author_name"><?php global $insoxin; if ($insoxin['switch_link_go']) {commentauthor();} else {comment_author_link();} ?></span><?php _e( '发布于：', 'insoxin' ); ?>&nbsp;<time class="datetime"><?php echo get_comment_date().'&nbsp;'.get_comment_time() ?></time>
            </div> <div class="comment_btn">  
                <?php if(get_post_type() != 'product'){ ?> 
                <?php edit_comment_link( __( '编辑', 'insoxin' ), '', '' ); ?>
                <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( '回复', 'insoxin' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
                <?php }else{ ?>
            ...
            <?php if ( $comment->comment_approved == '0' ) : ?>
                <div class="comment_awaiting_moderation"><?php _e( '评论已提交请等待审核通过。', 'insoxin' ); ?></div>
            <?php endif; ?>
		</article>
	<?php break; endswitch;
}
...
if(!function_exists('fa_ajax_comment_callback')) :
    ...
    function fa_ajax_comment_callback(){
        ...
        global $insoxin,$product,$woocommerce;
        ?>
        <li <?php comment_class(); ?>>
            <article id="comment-<?php comment_ID(); ?>" class="comment_body">
                <div class="comment_author">
                ...
                <div class="comment_info">
                    <?php
                        printf( __( '%1$s 发布于 %2$s', 'insoxin' ),
                            sprintf( '<span class="author_name">%s</span>', get_comment_author_link() ),
                            sprintf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
                                esc_url( get_comment_link( $comment->comment_ID ) ),
                                get_comment_time( 'c' ),
                                sprintf( __( '%1$s %2$s', 'insoxin' ), get_comment_date(), get_comment_time() )
                            )
                        );
                    ?>
                    <?php if (class_exists('woocommerce')){ do_action( 'woocommerce_review_before_comment_meta', $comment ); } ?>
